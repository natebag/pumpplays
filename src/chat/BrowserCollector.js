                        ...
                        mutation.addedNodes.forEach((node) => {
                            if (node.nodeType === 1) { // Element node
                                ...
                                if (chatMessage) {
                                    ...
                                    console.log('Full chat message HTML:', chatMessage.outerHTML);
                                    ...
                                    let usernameLink = chatMessage.querySelector('a[href*="/profile/"]');
                                    if (!usernameLink) {
                                        usernameLink = chatMessage.querySelector('a[href^="/profile/"]');
                                    }
                                    if (!usernameLink) {
                                        usernameLink = chatMessage.querySelector('.username, .user-name, [data-username]');
                                    }
                                    if (!usernameLink) {
                                        usernameLink = chatMessage.querySelector('span:first-child, div:first-child');
                                    }
                                    
                                    console.log('Username element found:', usernameLink ? usernameLink.outerHTML : 'NONE');
                                    ...
                                    if (messageText) {
                                        ...
                                        if (text.match(/^[!/]/) || text.match(/[!/]/)) {
                                            const commandMatch = text.match(/[!/](?:hold)?[a-z]+(?:\d+)?/gi);
                                            if (commandMatch) {
                                                commandMatch.forEach(cmd => {
                                                    ...
                                                    // Actually emit the command to the voting system
                                                    const cleanCommand = cmd.replace(/^[!/]/, '').toLowerCase();
                                                    this.emit('message', `!${cleanCommand}`, user);
                                                });
                                            }
                                        }
