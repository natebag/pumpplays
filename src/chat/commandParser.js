...
class CommandParser {
    ...
    parseCommand(message) {
        ...
        // Hold commands: !holdleft500 or !holda800 or !holdleft (uses default ms)
        const holdMatch = command.match(/^hold([a-z]+?)(\d+)?$/);
        if (holdMatch) {
            const key = holdMatch[1];
            let ms;
            if (holdMatch[2]) {
                ms = parseInt(holdMatch[2]);
            } else {
                // Special case: holdright gets 2000ms default, others get 4000ms
                ms = (key === 'right') ? 2000 : this.config.holdDefaultsMs;
            }
