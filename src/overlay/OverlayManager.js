...
class OverlayManager extends EventEmitter {
    ...
    /**
     * Update party Pokemon data
     * @param {Array} partyData - Array of Pokemon objects
     */
    updateParty(partyData) {
        this.state.party.pokemon = partyData || [];
        this.state.party.lastUpdated = Date.now();
        this.state.party.scanning = false;
        
        // Emit party update to overlay
        this.io?.emit('partyUpdate', {
            pokemon: this.state.party.pokemon,
            lastUpdated: this.state.party.lastUpdated
        });
        
        log(`Party updated: ${partyData.length} Pokemon`, 'OVERLAY');
    }
