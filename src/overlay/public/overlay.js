...
class PokemonOverlay {
    constructor() {
        ...
        this.commands = ['up', 'down', 'left', 'right', 'a', 'b', 'l', 'r', 'start', 'select', 'party'];
        this.chatMessages = [];
    ...
    connectSocket() {
        ...
        this.socket.on('partyUpdate', (partyData) => {
            this.updatePartyDisplay(partyData);
        });
    }
    ...
    updatePartyDisplay(partyData) {
        ...
        if (!partyContainer || !partyStatus) return;
        
        // Update party state
        this.state.party.pokemon = partyData.pokemon || [];
        this.state.party.lastUpdated = partyData.lastUpdated;
        this.state.party.scanning = partyData.scanning || false;
        
        // Update status indicator
        if (this.state.party.scanning) {
            partyStatus.textContent = 'Scanning...';
            partyStatus.className = 'party-status scanning';
        } else if (this.state.party.pokemon.length > 0) {
            partyStatus.textContent = `${this.state.party.pokemon.length} Pokemon`;
            partyStatus.className = 'party-status';
        } else {
            partyStatus.textContent = 'Ready';
            partyStatus.className = 'party-status';
        }
        
        // Update party display
        if (this.state.party.pokemon.length === 0) {
            partyContainer.innerHTML = '<div class="no-party">Use !party command to scan your team!</div>';
        } else {
            partyContainer.innerHTML = this.state.party.pokemon.map((pokemon, index) => {
                const hpPercent = pokemon.maxHp > 0 ? Math.floor((pokemon.hp / pokemon.maxHp) * 100) : 100;
                const statusDisplay = pokemon.status && pokemon.status !== 'OK' ? pokemon.status : '';
                
                return `
                    <div class="pokemon-item">
                        <span class="pokemon-slot">#${pokemon.slot || index + 1}</span>
                        <span class="pokemon-name">${pokemon.name || 'Unknown'}</span>
                        <span class="pokemon-level">Lv.${pokemon.level || '?'}</span>
                        <span class="pokemon-hp">${pokemon.hp || '?'}/${pokemon.maxHp || '?'} (${hpPercent}%)</span>
                        ${statusDisplay ? `<span class="pokemon-status">${statusDisplay}</span>` : ''}
                    </div>
                `;
            }).join('');
        }
